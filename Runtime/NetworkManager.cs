    ...
    public class NetworkManager : Singleton<NetworkManager>
    {
            ...
            int targetSceneID, int selectionGroup, int selectionNo, string selectionData)
        {
            JsonData sending = new JsonData
            {
                ...
                [CommonConst.FUNC] = "updateUserSelectionProgress"
            };
        ...
        public void UpdateUserProjectCurrent(int episodeID, int sceneID, long scriptNo, bool showLoadingBar)
        {
            ...
            JsonData sending = new JsonData();
            sending[CommonConst.COL_PROJECT_ID] = StoryManager.Instance.currentProjectID; // 현재 프로젝트 
            sending["episode_id"] = episodeID;
            sending["scene_id"] = sceneID;
            sending["script_no"] = scriptNo;
            sending[CommonConst.FUNC] = "updateUserProject"; // func 지정 
        ...
        public bool UpdateVisualCollection(int resourceID, string visualType)
        {
            ...
            JsonData sending = new JsonData
            {
                ...
                [CommonConst.FUNC] = "addGalleryIllust"
            };
        ...
        public void GetUnreadMailList(OnRequestFinishedDelegate cb = null, bool showLoadingBar = true)
        {
            ...
            {
                [CommonConst.FUNC] = "getUserMailList"
            };
        ...
        {
            jsonData[CommonConst.COL_USERKEY] = UserManager.Instance.userKey;
            jsonData["build"] = Application.identifier;
            jsonData["country"] = "XX";
            jsonData[LobbyConst.COL_LANG] = SystemManager.Instance.currentAppLanguageCode;
            jsonData["os"] = "Android";
        ...
        {
            string requestURL = url + CommonConst.CLIENT_URL;
            sendingData ??= new JsonData();

            SetBaseParams(sendingData);

            if (sendingData.ContainsKey(CommonConst.FUNC) && addList)
            {
                _listNetwork.Add(sendingData[CommonConst.FUNC].ToString());
            }
            ...
            HTTPRequest request, HTTPResponse response, bool dontHideNetworkLoading = false)
        {
            ...
            try
            {
                if (request.RawData != null)
                {
                        ...
                        (requestRawData.StartsWith("[") && requestRawData.EndsWith("]")))
                    {
                        ...
                        if (reqData != null && reqData.ContainsKey(CommonConst.FUNC))
                        {
                        }
                    }
                }
            ...
            switch (request.State)
            {
                case HTTPRequestStates.Finished:
                    ...
                    if (reqData != null && reqData.ContainsKey(CommonConst.FUNC))
                    {
                        Instance._listNetwork.Remove(reqData[CommonConst.FUNC].ToString());
                    }
